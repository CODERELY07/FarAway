...
if (!isset($_GET['category_id']) || !is_numeric($_GET['category_id'])) {
    echo '<p class="text-center text-gray-500">Invalid category selected.</p>';
    exit;
}

$categoryId = (int) $_GET['category_id'];

$stmt = $conn->prepare("SELECT * FROM properties WHERE category_id = :category_id");
...
if (count($properties) > 0) {
    foreach ($properties as $property) {
        ...
        ?>
        <a href="./views/user/property_details.php?id=<?= $property['property_id'] ?>" class="block h-full">
          <article class="flex flex-col max-w-sm h-full overflow-hidden rounded-xl text-gray-700 shadow-md duration-300 ease-in-out hover:shadow-xl cursor-pointer bg-white">
            <div class="flex-shrink-0">
              <img src="<?= $photo ?>" alt="Property Front photo"
    ...
    echo '<p class="text-center text-gray-500">No properties found for this category.</p>';
}
